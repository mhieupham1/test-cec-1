"""
Dashboard Service - Aggregates user statistics and activity data
"""
import logging
...
class DashboardService:
    ...
    async def get_dashboard_data(self, user_id: str) -> dict:
        ...
        data = {
            "user": {"name": user["name"], "avatar": user.get("avatar_url")},
    ...
    async def _get_recent_projects(self, user_id: str, limit: int = 5) -> list:
        projects = (
            await self.db.projects.find({"members": user_id, "is_deleted": False})
            .sort("updated_at", -1)
            .limit(limit)
